function examenF() {
    // Variables
    let InversionI = document.querySelector("#money").value;
    ...
    if (InversionI == "") {
      ...
      // Validacion extra
      if (lector.test(InversionI)) {
        alert("Ingresa datos validos");
        return false;
      } else {
        // Parsing variables and new variables
        InversionI = parseFloat(InversionI);
        tiempo = parseFloat(tiempo);
        let tasaAnual = 0.96;
        let tasaMensual = tasaAnual / 12;
        let tiempoMeses = tiempo * 12;
        let monto = InversionI / tiempoMeses;
        let tiempo = "";
        let montoI = "";
        let interes;
        let restante = InversionI;
        let cuota;
        let cuotaI = "";
        let interesI = "";
        let restanteI = "";
        ...
        // Calculos
        if (InversionI > 0 && InversionI < 9999999) {
